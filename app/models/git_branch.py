...
class GitBranch(db.Model):
    ...
    git_repository_id = db.Column(db.Integer,
                           db.ForeignKey('git_repository.id', ondelete='CASCADE'),  
                           nullable=False)
    latest_commits = db.Column(db.String, nullable=False)

    project = db.relationship('Project', back_populates='branches')
    collaborator_access = db.relationship('CollaboratorAccess', back_populates='branch', uselist=False, cascade='all, delete-orphan')

...
def add_2_database(self, user, proj_name, proj_url, description=None, access_token=None):
    ...
    try:
        dir_path = os.path.join(app.config['STATIC_FOLDER_1'], "repository", uuid.UUID(str(task_id)).hex)
        ...
        repo = add_to_database_repository(task_id, proj_url, default_branch, dir_path, access_token)
...
def add_to_database_repository(task_id, proj_url, default_branch, dir_path, access_token=None):
